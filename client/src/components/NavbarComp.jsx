...
<Navbar
    fluid
    rounded
    className={`fixed top-0 left-0 right-0 rounded-lg justify-between w-full max-w-full z-50 overflow-hidden bg-transparent transition-transform duration-300 ${
      scrollPosition > 0 ? (isNavbarVisible ? 'translate-y-0 bg-white-800/50' : '-translate-y-full') : 'mt-0'
    }`}
    style={{ boxShadow: '0px 4px 10px rgb(0 0 0 / 0%)' }}
  >

      <div className={`flex flex-wrap rounded-lg md:flex-nowrap pb-6 w-full items-center justify-between max-w-full overflow-hidden`}>
        <Navbar.Brand href="/">
